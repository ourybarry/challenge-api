...
/**
 * This authenticator works the same as the other authenticator, except that this one get the user token from request body,
 * this authenticator is used for the file download route
 */
class TokenPOSTAuthenticator extends AbstractAuthenticator{
    static public $TOKEN_KEY = 'jwtToken';
    ...
    {
        return $request->request->has(TokenPOSTAuthenticator::$TOKEN_KEY);
    }
    public function authenticate(Request $request): Passport
    {
        $token = $request->request->get(TokenPOSTAuthenticator::$TOKEN_KEY);
        if(null === $token){
